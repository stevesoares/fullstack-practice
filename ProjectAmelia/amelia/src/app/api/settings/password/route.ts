...
export async function POST(req: Request) {
  ...
  if (!newPassword || !confirmPassword) return Response.json({ message: "Missing fields" }, { status: 400 });
  ...
  if (user.passwordHash) {
    if (!currentPassword) return Response.json({ message: "Missing current password" }, { status: 400 });
